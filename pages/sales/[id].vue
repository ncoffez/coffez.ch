  ...
  <div
    ...
    <div id="text">
      ...
      <hgroup>
        ...
        <UiPayWithStripe
          :amount="350"
          :description="`Caricature bought at ${currentURL}.`"
          v-if="!(isPaid === 'true')"></UiPayWithStripe>
      </hgroup>
    </div>
    <div id="image" class="max-w-[500px]" :class="isPaid === 'true' ? '' : 'blur-sm'">
      <img v-if="image.urlFirebaseOriginalWithQR" :src="image.urlFirebaseOriginalWithQR" :alt="image.id" srcset="" />
...
const isPaid = ref(route.query.isPaid || "not Paid");
...
const { data: event } = await useFetch<any>("/api/getEvent", {
  method: "POST",
