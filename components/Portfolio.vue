<template>
  <section id="portfolio" class="w-full max-w-screen-xl" v-if="images">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-6 gap-3 md:gap-4 lg:gap-6 xl:gap-8"> 
      <div v-for="(image, index) of images" class="cursor-pointer w-full aspect-[1.5]" @click="router.push(`/sales/${image.id}`)">
        <picture>
          <source :srcset="image.urlFirebaseWebp" type="image/webp" />
          <source :srcset="image.urlFirebaseJpg" type="image/jpeg" />
          <img class="rounded-md w-full h-full object-cover" :src="image.urlFirebaseJpg" :alt="`Portrait${index + 1}`" srcset="" />
        </picture>
      </div>
    </div>
  </section>
